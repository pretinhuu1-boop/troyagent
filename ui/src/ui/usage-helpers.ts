...
export const filterSessionsByQuery = <TSession extends UsageSessionQueryTarget>(
  ...
  for (const term of terms) {
    ...
    if (!QUERY_KEYS.has(normalizedKey)) {
      warnings.push(`Filtro desconhecido: ${term.key}`);
      continue;
    }
    if (term.value === "") {
      warnings.push(`Valor ausente para ${term.key}`);
    }
    if (normalizedKey === "has") {
      ...
      if (term.value && !allowed.has(normalizeQueryText(term.value))) {
        warnings.push(`has:${term.value} desconhecido`);
      }
    }
    if (
      ...
      if (term.value && parseQueryNumber(term.value) === null) {
        warnings.push(`Número inválido para ${term.key}`);
      }
    }
  }
...
export function parseToolSummary(content: string) {
  ...
  const summary =
    sortedTools.length > 0
      ? `Ferramentas: ${sortedTools
        .map(([name, count]) => `${name}×${count}`)
        .join(", ")} (${totalCalls} chamadas)`
      : "";
  return {
    tools: sortedTools,
    summary,
