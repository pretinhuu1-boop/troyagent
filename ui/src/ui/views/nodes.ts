  ...
  return html`
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">Nós</div>
          <div class="card-sub">Dispositivos pareados e links ativos.</div>
        </div>
      ...
      ? html`
                <div class="muted">Nenhum nó encontrado.</div>
              `
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          ...
          <div class="card-sub">Solicitações de pareamento + tokens de função.</div>
        </div>
        <button class="btn" ?disabled=${props.devicesLoading} @click=${props.onDevicesRefresh}>
          ${props.devicesLoading ? "Carregando…" : "Atualizar"}
        </button>
      </div>
      ${props.devicesError
      ...
      <div class="list" style="margin-top: 16px;">
        ${pending.length > 0
      ? html`
              <div class="muted" style="margin-bottom: 8px;">Pendente</div>
      ...
      : nothing
        ...
        ${paired.length > 0
      ? html`
              <div class="muted" style="margin-top: 12px; margin-bottom: 8px;">Pareado</div>
...
function renderPendingDevice(req: PendingDevice, props: NodesProps) {
  ...
  const role = req.role?.trim() ? `função: ${req.role}` : "função: -";
  const repair = req.isRepair ? " · reparo" : "";
...
function renderPairedDevice(device: PairedDevice, props: NodesProps) {
  ...
  const roles = `funções: ${formatList(device.roles)}`;
  const scopes = `escopos: ${formatList(device.scopes)}`;
  ...
  return html`
    <div class="list-item">
      <div class="list-main">
        ...
        ${tokens.length === 0
      ? html`
                <div class="muted" style="margin-top: 6px">Tokens: nenhum</div>
              `
      : html`
...
function renderTokenRow(deviceId: string, token: DeviceTokenSummary, props: NodesProps) {
  const status = token.revokedAtMs ? "revogado" : "ativo";
  const scopes = `escopos: ${formatList(token.scopes)}`;
...
const SECURITY_OPTIONS: Array<{ value: ExecSecurity; label: string }> = [
  ...
  { value: "allowlist", label: "Lista de Permissão" },
  { value: "full", label: "Completo" },
];

const ASK_OPTIONS: Array<{ value: ExecAsk; label: string }> = [
  { value: "off", label: "Desligado" },
  { value: "on-miss", label: "Se faltar" },
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        <div>
          <div class="card-title">Vínculo de nó de execução</div>
          <div class="card-sub">
            Fixe agentes em um nó específico ao usar <span class="mono">exec host=node</span>.
          </div>
        </div>
        <button
          class="btn"
          ...
          @click=${state.onSave}
        >
          ${state.configSaving ? "Salvando…" : "Salvar"}
        </button>
      </div>
      ...
      ? html`
              <div class="callout warn" style="margin-top: 12px">
                Mude a aba de Configuração para o modo <strong>Formulário</strong> para editar os vínculos aqui.
              </div>
            `
      : nothing
    }
      ...
      ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            <div class="muted">Carregue a configuração para editar os vínculos.</div>
            <button class="btn" ?disabled=${state.configLoading} @click=${state.onLoadConfig}>
              ${state.configLoading ? "Carregando…" : "Carregar configuração"}
            </button>
          </div>`
      : html`
            <div class="list" style="margin-top: 16px;">
              <div class="list-item">
                <div class="list-main">
                  <div class="list-title">Vínculo padrão</div>
                  <div class="list-sub">Usado quando os agentes não substituem um vínculo de nó.</div>
                </div>
                <div class="list-meta">
                  <label class="field">
                    <span>Nó</span>
          ...
          state.onBindDefault(value ? value : null);
                    ...
                    >
                      <option value="" ?selected=${defaultValue === ""}>Qualquer nó</option>
                      ${state.nodes.map(
          (node) =>
            html`<option
                            value=${node.id}
                  ...
                  ${!supportsBinding
          ? html`
                          <div class="muted">Nenhum nó com system.run disponível.</div>
                        `
          : nothing
        }
                </div>
...
function renderExecApprovals(state: ExecApprovalsState) {
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        <div>
          <div class="card-title">Aprovações de execução</div>
          <div class="card-sub">
            Lista de permissões e política de aprovação para <span class="mono">exec host=gateway/node</span>.
          </div>
        </div>
        <button
          class="btn"
      ...
      ${!ready
      ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            <div class="muted">Carregue as aprovações de execução para editar as listas de permissão.</div>
            <button class="btn" ?disabled=${state.loading || !targetReady} @click=${state.onLoad}>
              ${state.loading ? "Carregando…" : "Carregar aprovações"}
            </button>
          </div>`
      : html`
...
function renderExecApprovalsTarget(state: ExecApprovalsState) {
  ...
  return html`
    <div class="list" style="margin-top: 12px;">
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">Alvo</div>
          <div class="list-sub">
            Gateway edita aprovações locais; o nó edita o nó selecionado.
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
      ...
      ? html`
          ...
          state.onSelectTarget("node", value ? value : null);
                  ...
                  >
                    <option value="" ?selected=${nodeValue === ""}>Selecionar nó</option>
      ...
      ? html`
              <div class="muted">Nenhum nó anunciou aprovações de execução ainda.</div>
            `
      : nothing
    }
    </div>
    ...
    <div class="list" style="margin-top: 16px;">
      <div class="list-item">
        <div class="list-main">
          ...
          <div class="list-sub">
            ${isDefaults ? "Modo de segurança padrão." : `Padrão: ${defaults.security}.`}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">Pergunta</div>
          <div class="list-sub">
            ${isDefaults ? "Política de prompt padrão." : `Padrão: ${defaults.ask}.`}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">Opção de fallback para Pergunta</div>
          <div class="list-sub">
            ${isDefaults
      ? "Aplicado quando o prompt da UI não está disponível."
      : `Padrão: ${defaults.askFallback}.`
    }
          </div>
        </div>
        <div class="list-meta">
      ...
      ? html`<option value="__default__" ?selected=${askFallbackValue === "__default__"}>
                    Usar padrão (${defaults.askFallback})
                  </option>`
      : nothing
    }
              ${SECURITY_OPTIONS.map(
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">Permitir automaticamente CLIs de habilidades</div>
          <div class="list-sub">
            ${isDefaults
      ? "Permitir executáveis de habilidades listados pelo Gateway."
      : autoIsDefault
        ? `Usando padrão (${defaults.autoAllowSkills ? "ligado" : "desligado"}).`
        : `Substituição (${autoEffective ? "ligado" : "desligado"}).`
    }
          </div>
        </div>
        <div class="list-meta">
...
function renderExecApprovalsAllowlist(state: ExecApprovalsState) {
  ...
  return html`
    <div class="row" style="margin-top: 18px; justify-content: space-between;">
      <div>
        <div class="card-title">Lista de Permissões</div>
        <div class="card-sub">Padrões glob (insensíveis a maiúsculas/minúsculas).</div>
      </div>
      <button
        class="btn btn--sm"
        ?disabled=${state.disabled}
    ...
    <div class="list" style="margin-top: 12px;">
      ${entries.length === 0
      ? html`
              <div class="muted">Nenhuma entrada na lista de permissões ainda.</div>
            `
...
function renderAllowlistEntry(
  ...
  return html`
    <div class="list-item">
      <div class="list-main">
        <div class="list-title">${entry.pattern?.trim() ? entry.pattern : "Novo padrão"}</div>
        <div class="list-sub">Último uso: ${lastUsed}</div>
  ...
  return html`
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="list-sub">
          ${agent.isDefault ? "agente padrão" : "agente"} ·
          ${bindingValue === "__default__"
      ? `usa o padrão (${state.defaultBinding ?? "qualquer"})`
      : `substituição: ${agent.binding}`
    }
        </div>
      </div>
      <div class="list-meta">
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="chip-row" style="margin-top: 6px;">
          <span class="chip">${paired ? "pareado" : "não pareado"}</span>
