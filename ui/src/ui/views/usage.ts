  ...
  return html`
    <div class="card usage-mosaic">
      ...
      <div class="usage-mosaic-grid">
        <div class="usage-mosaic-section">
          <div class="usage-mosaic-section-title">Dia da Semana</div>
    ...
    return html`
          ...
          <div class="usage-hour-legend">
            <span></span>
            Baixa → Alta densidade de tokens
          </div>
        </div>
      </div>
    </div>
  ...
  return html`
    <div class="daily-chart-compact">
      <div class="daily-chart-header">
        ...
        <div class="card-title">Uso Diário de ${isTokenMode ? "Tokens" : "Custo"}</div>
      </div>
    ...
    const breakdownLines =
      dailyChartMode === "by-type"
        ? isTokenMode
          ? [
            `Saída ${formatTokens(d.output)}`,
            `Entrada ${formatTokens(d.input)}`,
            `Escrita em cache ${formatTokens(d.cacheWrite)}`,
            `Leitura de cache ${formatTokens(d.cacheRead)}`,
          ]
          : [
            `Saída ${formatCost(d.outputCost ?? 0)}`,
            `Entrada ${formatCost(d.inputCost ?? 0)}`,
            `Escrita em cache ${formatCost(d.cacheWriteCost ?? 0)}`,
            `Leitura de cache ${formatCost(d.cacheReadCost ?? 0)}`,
          ]
        : [];
...
function renderCostBreakdownCompact(totals: UsageTotals, mode: "tokens" | "cost") {
  ...
  return html`
    <div class="cost-breakdown cost-breakdown-compact">
      <div class="cost-breakdown-header">${isTokenMode ? "Tokens" : "Custo"} por Tipo</div>
      <div class="cost-breakdown-bar">
        <div class="cost-segment output" style="width: ${(isTokenMode ? tokenPcts.output : breakdown.output.pct).toFixed(1)}%"
          title="Saída: ${isTokenMode ? formatTokens(totals.output) : formatCost(breakdown.output.cost)}"></div>
        <div class="cost-segment input" style="width: ${(isTokenMode ? tokenPcts.input : breakdown.input.pct).toFixed(1)}%"
          title="Entrada: ${isTokenMode ? formatTokens(totals.input) : formatCost(breakdown.input.cost)}"></div>
        <div class="cost-segment cache-write" style="width: ${(isTokenMode ? tokenPcts.cacheWrite : breakdown.cacheWrite.pct).toFixed(1)}%"
          title="Escrita em Cache: ${isTokenMode ? formatTokens(totals.cacheWrite) : formatCost(breakdown.cacheWrite.cost)}"></div>
        <div class="cost-segment cache-read" style="width: ${(isTokenMode ? tokenPcts.cacheRead : breakdown.cacheRead.pct).toFixed(1)}%"
          title="Leitura de Cache: ${isTokenMode ? formatTokens(totals.cacheRead) : formatCost(breakdown.cacheRead.cost)}"></div>
      </div>
      <div class="cost-breakdown-legend">
        <span class="legend-item"><span class="legend-dot output"></span>Saída ${isTokenMode ? formatTokens(totals.output) : formatCost(breakdown.output.cost)}</span>
        <span class="legend-item"><span class="legend-dot input"></span>Entrada ${isTokenMode ? formatTokens(totals.input) : formatCost(breakdown.input.cost)}</span>
        <span class="legend-item"><span class="legend-dot cache-write"></span>Escrita Cache ${isTokenMode ? formatTokens(totals.cacheWrite) : formatCost(breakdown.cacheWrite.cost)}</span>
        <span class="legend-item"><span class="legend-dot cache-read"></span>Leitura Cache ${isTokenMode ? formatTokens(totals.cacheRead) : formatCost(breakdown.cacheRead.cost)}</span>
      </div>
...
function renderUsageInsights(
  ...
  const cacheHint = "Taxa de acerto de cache = leitura de cache / (entrada + leitura de cache). Quanto maior, melhor.";
  const errorHint = "Taxa de erro = erros / total de mensagens. Quanto menor, melhor.";
  const throughputHint = "Vazão mostra tokens por minuto ao longo do tempo ativo. Quanto maior, melhor.";
  const tokensHint = "Média de tokens por mensagem neste intervalo.";
  const costHint = showCostHint
    ? "Custo médio por mensagem quando provedores relatam custos. Dados de custo ausentes para algumas ou todas as sessões neste intervalo."
    : "Custo médio por mensagem quando provedores relatam custos.";
  ...
  const errorDays = aggregates.daily
    ...
    .map((day) => {
      ...
      return {
        ...
        sub: `${day.errors} erros · ${day.messages} msgs · ${formatTokens(day.tokens)}`,
        rate,
      };
    })
    .toSorted((a, b) => b.rate - a.rate)
  ...
  const topModels = aggregates.byModel.slice(0, 5).map((entry) => ({
    label: entry.model ?? "desconhecido",
  ...
  const topProviders = aggregates.byProvider.slice(0, 5).map((entry) => ({
    label: entry.provider ?? "desconhecido",
    value: formatCost(entry.totals.totalCost),
    sub: `${formatTokens(entry.totals.totalTokens)} · ${entry.count} msgs`,
  }));
  ...
  return html`
    <section class="card" style="margin-top: 16px;">
      <div class="card-title">Visão Geral de Uso</div>
      <div class="usage-summary-grid">
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            Mensagens
            <span class="usage-summary-hint" title="Total de mensagens de usuário + assistente no intervalo.">?</span>
          </div>
          <div class="usage-summary-value">${aggregates.messages.total}</div>
          <div class="usage-summary-sub">
            ${aggregates.messages.user} usuário · ${aggregates.messages.assistant} assistente
          </div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            Chamadas de Ferr.
            <span class="usage-summary-hint" title="Total de chamadas de ferramenta nas sessões.">?</span>
          </div>
          <div class="usage-summary-value">${aggregates.tools.totalCalls}</div>
          <div class="usage-summary-sub">${aggregates.tools.uniqueTools} ferramentas</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            Erros
            <span class="usage-summary-hint" title="Total de erros de mensagem/ferramenta no intervalo.">?</span>
          </div>
          <div class="usage-summary-value">${aggregates.messages.errors}</div>
          <div class="usage-summary-sub">${aggregates.messages.toolResults} resultados</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            Méd. Tokens / Msg
          ...
          <div class="usage-summary-sub">Em ${aggregates.messages.total || 0} mensagens</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            Méd. Custo / Msg
        ...
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            Sessões
            <span class="usage-summary-hint" title="Sessões distintas no intervalo.">?</span>
          </div>
          <div class="usage-summary-value">${sessionCount}</div>
          <div class="usage-summary-sub">de ${totalSessions} no intervalo</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            Vazão
        ...
        <div class="usage-summary-card">
          ...
          <div class="usage-summary-sub">
            ${aggregates.messages.errors} erros · ${avgDurationLabel} méd. sessão
          </div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
          ...
          <div class="usage-summary-sub">
            ${formatTokens(totals.cacheRead)} em cache · ${formatTokens(cacheBase)} prompt
          </div>
        </div>
      </div>
      <div class="usage-insights-grid">
        ${renderInsightList("Principais Modelos", topModels, "Sem dados de modelo")}
        ${renderInsightList("Principais Provedores", topProviders, "Sem dados de provedor")}
        ${renderInsightList("Principais Ferramentas", topTools, "Sem chamadas de ferramenta")}
        ${renderInsightList("Principais Agentes", topAgents, "Sem dados de agente")}
        ${renderInsightList("Principais Canais", topChannels, "Sem dados de canal")}
        ${renderPeakErrorList("Dias de Pico de Erro", errorDays, "Sem dados de erro")}
        ${renderPeakErrorList("Horas de Pico de Erro", errorHours, "Sem dados de erro")}
      </div>
    </section>
  `;
}
    ...
    <div class="card sessions-card">
      <div class="sessions-card-header">
        ...
        <div class="sessions-card-count">
          ${sessions.length} exibidos${totalSessions !== sessions.length ? ` · ${totalSessions} total` : ""}
        </div>
      </div>
      <div class="sessions-card-meta">
        <div class="sessions-card-stats">
          <span>${isTokenMode ? formatTokens(avgValue) : formatCost(avgValue)} méd</span>
        ...
        <label class="sessions-sort">
          ...
          >
            <option value="cost" ?selected=${sessionSort === "cost"}>Custo</option>
            <option value="errors" ?selected=${sessionSort === "errors"}>Erros</option>
            <option value="messages" ?selected=${sessionSort === "messages"}>Mensagens</option>
            <option value="recent" ?selected=${sessionSort === "recent"}>Recente</option>
        ...
        <button
          ...
          title=${sessionSortDir === "desc" ? "Decrescente" : "Crescente"}
        >
          ${sessionSortDir === "desc" ? "↓" : "↑"}
        </button>
        ${selectedCount > 0
      ...
      ? recentEntries.length === 0
        ? html`
                <div class="muted" style="padding: 20px; text-align: center">Nenhuma sessão recente</div>
              `
        : html`
      ...
      : sessions.length === 0
        ? html`
                <div class="muted" style="padding: 20px; text-align: center">Nenhuma sessão no intervalo</div>
              `
        : html`
          ...
          return html`
              ...
              void copySessionName(s);
                  ...
                  ${sessions.length > 50 ? html`<div class="muted" style="padding: 8px; text-align: center; font-size: 11px;">+${sessions.length - 50} mais</div>` : nothing}
                </div>
              `
    }
      ${selectedCount > 1
      ? html`
              <div style="margin-top: 10px;">
                <div class="sessions-card-count">Selecionado(s) (${selectedCount})</div>
...
function renderSessionSummary(session: UsageSessionEntry) {
  ...
  if (!usage) {
    return html`
      <div class="muted">Sem dados de uso para esta sessão.</div>
    `;
  }
  ...
  return html`
    ...
    <div class="session-summary-grid">
      <div class="session-summary-card">
        <div class="session-summary-title">Mensagens</div>
        <div class="session-summary-value">${usage.messageCounts?.total ?? 0}</div>
        <div class="session-summary-meta">${usage.messageCounts?.user ?? 0} usuário · ${usage.messageCounts?.assistant ?? 0} assistente</div>
      </div>
      <div class="session-summary-card">
        <div class="session-summary-title">Chamadas de Ferr.</div>
        <div class="session-summary-value">${usage.toolUsage?.totalCalls ?? 0}</div>
        <div class="session-summary-meta">${usage.toolUsage?.uniqueTools ?? 0} ferramentas</div>
      </div>
      <div class="session-summary-card">
        <div class="session-summary-title">Erros</div>
        <div class="session-summary-value">${usage.messageCounts?.errors ?? 0}</div>
        <div class="session-summary-meta">${usage.messageCounts?.toolResults ?? 0} resultados de ferr.</div>
      </div>
      <div class="session-summary-card">
        <div class="session-summary-title">Duração</div>
    ...
    <div class="usage-insights-grid" style="margin-top: 12px;">
      ${renderInsightList("Principais Ferramentas", toolItems, "Sem chamadas de ferramenta")}
      ${renderInsightList("Mix de Modelos", modelItems, "Sem dados de modelo")}
    </div>
  `;
}
  ...
  return html`
    <div class="card session-detail-panel">
      ...
      : nothing
        ...
        <button class="session-close-btn" @click=${onClose} title="Fechar detalhes da sessão">×</button>
      </div>
...
function renderTimeSeriesCompact(
  ...
  if (loading) {
    return html`
      <div class="session-timeseries-compact">
        <div class="muted" style="padding: 20px; text-align: center">Carregando...</div>
      </div>
    `;
  }
  if (!timeSeries || timeSeries.points.length < 2) {
    return html`
      <div class="session-timeseries-compact">
        <div class="muted" style="padding: 20px; text-align: center">Sem dados de linha do tempo</div>
      </div>
    `;
  }
  ...
  if (points.length < 2) {
    return html`
      <div class="session-timeseries-compact">
        <div class="muted" style="padding: 20px; text-align: center">Sem dados no intervalo</div>
      </div>
    `;
  }
  let cumTokens = 0,
  ...
  return html`
    <div class="session-timeseries-compact">
      <div class="timeseries-header-row">
        <div class="card-title" style="font-size: 13px;">Uso ao Longo do Tempo</div>
      ...
      if (breakdownByType) {
        tooltipLines.push(`Saída ${formatTokens(p.output)}`);
        tooltipLines.push(`Entrada ${formatTokens(p.input)}`);
        tooltipLines.push(`Escrita em cache ${formatTokens(p.cacheWrite)}`);
        tooltipLines.push(`Leitura de cache ${formatTokens(p.cacheRead)}`);
      }
      ...
      ? html`
              <div style="margin-top: 8px;">
                <div class="card-title" style="font-size: 12px; margin-bottom: 6px;">Tokens por Tipo</div>
                ...
                <div class="cost-breakdown-legend">
                  <div class="legend-item" title="Tokens de saída do assistente">
                    <span class="legend-dot output"></span>Saída ${formatTokens(sumOutput)}
                  </div>
                  <div class="legend-item" title="Tokens de entrada de usuário + ferramenta">
                    <span class="legend-dot input"></span>Entrada ${formatTokens(sumInput)}
                  </div>
                  <div class="legend-item" title="Tokens escritos em cache">
                    <span class="legend-dot cache-write"></span>Escrita Cache ${formatTokens(sumCacheWrite)}
                  </div>
                  <div class="legend-item" title="Tokens lidos de cache">
                    <span class="legend-dot cache-read"></span>Leitura Cache ${formatTokens(sumCacheRead)}
                  </div>
                </div>
...
function renderContextPanel(
  ...
  if (!contextWeight) {
    return html`
      <div class="context-details-panel">
        <div class="muted" style="padding: 20px; text-align: center">Sem dados de contexto</div>
      </div>
    `;
  }
  ...
  if (usage && usage.totalTokens > 0) {
    ...
    if (inputTokens > 0) {
      contextPct = `~${Math.min((totalContextTokens / inputTokens) * 100, 100).toFixed(0)}% da entrada`;
    }
  }
  ...
  return html`
    <div class="context-details-panel">
      <div class="context-breakdown-header">
        <div class="card-title" style="font-size: 13px;">Detalhamento do Prompt do Sistema</div>
        ${hasMore
      ? html`<button class="context-expand-btn" @click=${onToggleExpanded}>
                ${showAll ? "Recolher" : "Expandir tudo"}
              </button>`
      : nothing
    }
      </div>
      <p class="context-weight-desc">${contextPct || "Contexto base por mensagem"}</p>
      <div class="context-stacked-bar">
        <div class="context-segment system" style="width: ${pct(systemTokens, totalContextTokens).toFixed(1)}%" title="Sistema: ~${formatTokens(systemTokens)}"></div>
        <div class="context-segment skills" style="width: ${pct(skillsTokens, totalContextTokens).toFixed(1)}%" title="Habilidades: ~${formatTokens(skillsTokens)}"></div>
        <div class="context-segment tools" style="width: ${pct(toolsTokens, totalContextTokens).toFixed(1)}%" title="Ferramentas: ~${formatTokens(toolsTokens)}"></div>
        <div class="context-segment files" style="width: ${pct(filesTokens, totalContextTokens).toFixed(1)}%" title="Arquivos: ~${formatTokens(filesTokens)}"></div>
      </div>
      <div class="context-legend">
        <span class="legend-item"><span class="legend-dot system"></span>Sis ~${formatTokens(systemTokens)}</span>
        <span class="legend-item"><span class="legend-dot skills"></span>Hab ~${formatTokens(skillsTokens)}</span>
        <span class="legend-item"><span class="legend-dot tools"></span>Ferr ~${formatTokens(toolsTokens)}</span>
        <span class="legend-item"><span class="legend-dot files"></span>Arq ~${formatTokens(filesTokens)}</span>
      </div>
      ...
      ? (() => {
        ...
        return html`
                  <div class="context-breakdown-card">
                    <div class="context-breakdown-title">Habilidades (${skillsList.length})</div>
          ...
          (s) => html`
                    ...
                    ${more > 0
            ? html`<div class="context-breakdown-more">+${more} mais</div>`
            : nothing
          }
                  </div>
                `;
      ...
      ? (() => {
        ...
        return html`
                  <div class="context-breakdown-card">
                    <div class="context-breakdown-title">Ferramentas (${toolsList.length})</div>
                    <div class="context-breakdown-list">
                      ${toolsTop.map(
          (t) => html`
                          <div class="context-breakdown-item">
      ...
      ? (() => {
        ...
        return html`
                  <div class="context-breakdown-card">
                    <div class="context-breakdown-title">Arquivos (${filesList.length})</div>
                    <div class="context-breakdown-list">
                      ${filesTop.map(
          (f) => html`
                          <div class="context-breakdown-item">
...
function renderSessionLogsCompact(
  ...
  if (loading) {
    return html`
      <div class="session-logs-compact">
        <div class="session-logs-header">Conversa</div>
        <div class="muted" style="padding: 20px; text-align: center">Carregando...</div>
      </div>
    `;
  }
  if (!logs || logs.length === 0) {
    return html`
      <div class="session-logs-compact">
        <div class="session-logs-header">Conversa</div>
        <div class="muted" style="padding: 20px; text-align: center">Nenhuma mensagem</div>
      </div>
    `;
  }
  ...
  const displayedCount =
    filters.roles.length > 0 || filters.tools.length > 0 || filters.hasTools || normalizedQuery
      ? `${filteredEntries.length} de ${logs.length}`
      : `${logs.length}`;
  ...
  return html`
    <div class="session-logs-compact">
      <div class="session-logs-header">
        <span>Conversa <span style="font-weight: normal; color: var(--text-muted);">(${displayedCount} mensagens)</span></span>
      ...
      onFilterRolesChange(
        ...
        >
          <option value="user" ?selected=${roleSelected.has("user")}>Usuário</option>
          <option value="assistant" ?selected=${roleSelected.has("assistant")}>Assistente</option>
          <option value="tool" ?selected=${roleSelected.has("tool")}>Ferramenta</option>
          <option value="toolResult" ?selected=${roleSelected.has("toolResult")}>Res. Ferramenta</option>
        </select>
        <select
          multiple
          size="4"
      ...
      <div class="session-logs-list">
        ...
        const roleLabel =
          log.role === "user" ? "Você" : log.role === "assistant" ? "Assistente" : "Ferramenta";
        return html`
      ...
      ? html`
                <div class="muted" style="padding: 12px">Nenhuma mensagem corresponde aos filtros.</div>
              `
      : nothing
    }
      </div>
...
export function renderUsage(props: UsageProps) {
        ...
        <div class="row" style="justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 12px;">
          <div style="flex: 1; min-width: 250px;">
            <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 2px;">
              <div class="card-title" style="margin: 0;">Uso de Tokens</div>
              <span style="
                display: inline-flex;
                align-items: center;
                gap: 6px;
  ...
  const renderFilterSelect = (key: string, label: string, options: string[]) => {
    ...
    return html`
      ...
      >
        ...
        : html`
                <span class="usage-filter-badge">Tudo</span>
                `
      }
        </summary>
  ...
  return html`
    ...
    <section class="usage-page-header">
      <div class="usage-page-title">Uso</div>
      <div class="usage-page-subtitle">Veja para onde os tokens vão, quando as sessões aumentam e o que impulsiona o custo.</div>
    </section>
    ...
    <section class="card usage-header ${props.headerPinned ? "pinned" : ""}">
      <div class="usage-header-row">
        <div class="usage-header-title">
          ...
          ${props.loading
      ? html`
                  <span class="usage-refresh-indicator">Carregando</span>
                `
      : nothing
    }
          ${isEmpty
      ? html`
                  <span class="usage-query-hint">Selecione um intervalo de datas e clique em Atualizar para carregar o uso.</span>
                `
      : nothing
    }
        </div>
      ...
      ? html`
                ...
                <span class="usage-metric-badge">
                  <strong>${formatCost(displayTotals.totalCost)}</strong> custo
                </span>
                <span class="usage-metric-badge">
                  <strong>${displaySessionCount}</strong>
                  sess${displaySessionCount !== 1 ? "ões" : "ão"}
                </span>
              `
      : nothing
    }
          <button
            ...
            title=${props.headerPinned ? "Desafixar filtros" : "Fixar filtros"}
            @click=${props.onToggleHeaderPinned}
          >
            ${props.headerPinned ? "Fixado" : "Fixar"}
          </button>
          <details
      ...
      window.addEventListener("click", onClick, true);
          ...
          >
            <summary class="usage-export-button">Exportar ▾</summary>
      ...
      if (e.key === "Enter") {
        ...
        props.onApplyQuery();
          ...
          <div class="usage-query-actions">
            ...
            >
              Filtrar (Lado do Cliente)
            </button>
            ${hasDraftQuery || hasQuery
      ? html`<button class="btn btn-sm usage-action-btn usage-secondary-btn" @click=${props.onClearQuery}>Limpar</button>`
      : nothing
    }
            <span class="usage-query-hint">
              ${hasQuery
      ? `${filteredSessions.length} de ${totalSessions} sessões correspondem`
      : `${totalSessions} sessões no intervalo`
    }
            </span>
          </div>
        </div>
        <div class="usage-filter-row">
          ${renderFilterSelect("agent", "Agente", agentOptions)}
          ${renderFilterSelect("channel", "Canal", channelOptions)}
          ${renderFilterSelect("provider", "Provedor", providerOptions)}
          ${renderFilterSelect("model", "Modelo", modelOptions)}
          ${renderFilterSelect("tool", "Ferramenta", toolOptions)}
          <span class="usage-query-hint">
            Dica: use filtros ou clique nas barras para filtrar os dias.
          </span>
        </div>
        ${queryTerms.length > 0
      ? html`
      ...
      ? html`
              <div class="callout warning" style="margin-top: 12px">
                Mostrando as primeiras 1.000 sessões. Diminua o intervalo de datas para resultados completos.
              </div>
            `
      : nothing
    }
