...
export function renderChannelConfigForm(props: ChannelConfigFormProps) {
  ...
  const normalized = analysis.schema;
    if (!normalized) {
      return html`
        <div class="callout danger">Schema indisponível. Use Raw.</div>
      `;
    }
    const node = resolveSchemaNode(normalized, ["channels", props.channelId]);
    if (!node) {
      return html`
        <div class="callout danger">Schema de configuração do canal indisponível.</div>
      `;
    }
    ...
    <div style="margin-top: 16px;">
      ${
        props.configSchemaLoading
          ? html`
              <div class="muted">Carregando schema de configuração...</div>
            `
