...
export function renderConfig(props: ConfigProps) {
  const validity = props.valid == null ? "desconhecido" : props.valid ? "válido" : "inválido";
    ...
    <div class="config-layout">
      ...
      <aside class="config-sidebar">
        <div class="config-sidebar__header">
          <div class="config-sidebar__title">Configurações</div>
          <span
            class="pill pill--sm ${validity === "válido" ? "pill--ok" : validity === "inválido" ? "pill--danger" : ""
    }"
            >${validity}</span
          >
        </div>
      ...
      : nothing
        ...
        <nav class="config-nav">
          ...
          >
            ...
            <span class="config-nav__label">Todas as Configurações</span>
          </button>
          ${allSections.map(
      (section) => html`
              <button
      ...
      ? html`
          ...
          : `${diff.length} alteração${diff.length !== 1 ? "s" : ""} não salva${diff.length !== 1 ? "s" : ""}`
        }</span
                  >
                `
      : html`
                    <span class="config-status muted">Sem alterações</span>
                  `
    }
          </div>
          <div class="config-actions__right">
            ...
            >
              ${props.loading ? "Carregando…" : "Recarregar"}
            </button>
            <button
            ...
            <button
              ...
              @click=${props.onApply}
            >
              ${props.applying ? "Aplicando…" : "Aplicar"}
            </button>
            <button
              class="btn btn--sm"
              ?disabled=${!canUpdate}
            ...
            >
              ${props.updating ? "Atualizando…" : "Atualizar"}
            </button>
          </div>
        </div>
      ...
      ? html`
              <details class="config-diff">
                <summary class="config-diff__summary">
                  <span
                    >Ver ${diff.length} alteração${diff.length !== 1 ? "s" : ""}
                    pendente${diff.length !== 1 ? "s" : ""}</span
                  >
                  <svg
      ...
      ? html`
          ...
          ? html`
                        <div class="callout danger" style="margin-top: 12px">
                          A visualização de formulário não suporta a edição segura de alguns campos. Use a opção "Código (Raw)" para evitar perda de entradas.
                        </div>
                      `
          : nothing
        }
