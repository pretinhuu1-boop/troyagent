  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">Instâncias Conectadas</div>
          <div class="card-sub">Beacons de presença do gateway e clientes.</div>
        </div>
      ...
      ? html`
                <div class="muted">Nenhuma instância reportada ainda.</div>
              `
...
function renderEntry(entry: PresenceEntry) {
  const lastInput = entry.lastInputSeconds != null ? `${entry.lastInputSeconds}s atrás` : "n/d";
  ...
  const scopesLabel =
    scopes.length > 0
      ? scopes.length > 3
        ? `${scopes.length} escopos`
        : `escopos: ${scopes.join(", ")}`
      : null;
  return html`
    <div class="list-item">
      <div class="list-main">
        <div class="list-title">${entry.host ?? "host desconhecido"}</div>
      ...
      <div class="list-meta">
        ...
        <div class="muted">Última entrada ${lastInput}</div>
        <div class="muted">Motivo ${entry.reason ?? ""}</div>
      </div>
    </div>
  `;
}
