...
const TOOL_SECTIONS = [
  {
    ...
    label: "Arquivos",
    tools: [
      { id: "read", label: "leitura", description: "Ler o conteúdo do arquivo" },
      { id: "write", label: "escrita", description: "Criar ou sobrescrever arquivos" },
      { id: "edit", label: "edição", description: "Fazer edições precisas" },
      { id: "apply_patch", label: "aplicar_patch", description: "Patch de arquivos (OpenAI)" },
    ],
  },
  {
    id: "runtime",
    label: "Runtime",
    tools: [
      { id: "exec", label: "execução", description: "Executar comandos do shell" },
      { id: "process", label: "processo", description: "Gerenciar processos em segundo plano" },
    ],
  },
  {
    id: "web",
    label: "Web",
    tools: [
      { id: "web_search", label: "busca_web", description: "Pesquisar na web" },
      { id: "web_fetch", label: "obter_web", description: "Obter conteúdo da web" },
    ],
  },
  {
    id: "memory",
    label: "Memória",
    tools: [
      { id: "memory_search", label: "busca_memoria", description: "Busca semântica" },
      { id: "memory_get", label: "obter_memoria", description: "Ler arquivos de memória" },
    ],
  },
  {
    id: "sessions",
    label: "Sessões",
    tools: [
      { id: "sessions_list", label: "listar_sessoes", description: "Listar sessões" },
      { id: "sessions_history", label: "historico_sessoes", description: "Histórico da sessão" },
      { id: "sessions_send", label: "enviar_sessao", description: "Enviar para a sessão" },
      { id: "sessions_spawn", label: "gerar_subagente", description: "Gerar subagente" },
      { id: "session_status", label: "status_sessao", description: "Status da sessão" },
    ],
  },
  {
    id: "ui",
    label: "Interface",
    tools: [
      { id: "browser", label: "navegador", description: "Controlar navegador web" },
      { id: "canvas", label: "tela", description: "Controlar telas (canvas)" },
    ],
  },
  {
    id: "messaging",
    label: "Mensageria",
    tools: [{ id: "message", label: "mensagem", description: "Enviar mensagens" }],
  },
  {
    id: "automation",
    label: "Automação",
    tools: [
      { id: "cron", label: "agendamento", description: "Agendar tarefas" },
      { id: "gateway", label: "gateway", description: "Controle do Gateway" },
    ],
  },
  {
    id: "nodes",
    label: "Nós",
    tools: [{ id: "nodes", label: "nós", description: "Nós + dispositivos" }],
  },
  {
    id: "agents",
    label: "Agentes",
    tools: [{ id: "agents_list", label: "listar_agentes", description: "Listar agentes" }],
  },
  {
    id: "media",
    label: "Mídia",
    tools: [{ id: "image", label: "imagem", description: "Compreensão de imagem" }],
  },
];
...
function agentBadgeText(agentId: string, defaultId: string | null) {
  return defaultId && agentId === defaultId ? "padrão" : null;
}
...
function buildAgentContext(
  ...
  return {
    ...
    identityName,
    identityEmoji,
    skillsLabel: skillFilter ? `${skillCount} selecionada(s)` : "todas as habilidades",
...
function buildModelOptions(configForm: Record<string, unknown> | null, current?: string | null) {
  ...
  if (current && !hasCurrent) {
    options.unshift({ value: current, label: `Atual (${current})` });
  }
  if (options.length === 0) {
    return html`
      <option value="" disabled>Nenhum modelo configurado</option>
    `;
  }
    ...
    <div class="agents-layout">
      <section class="card agents-sidebar">
        <div class="row" style="justify-content: space-between;">
          <div>
            ...
            <div class="card-sub">${agents.length} configurado(s).</div>
          </div>
      ...
      : nothing
        ...
        <div class="agent-list" style="margin-top: 12px;">
          ${agents.length === 0
      ? html`
                  <div class="muted">Nenhum agente encontrado.</div>
                `
      : agents.map((agent) => {
      ...
      ? html`
                <div class="card">
                  <div class="card-title">Selecione um agente</div>
                  <div class="card-sub">Escolha um agente para inspecionar seu workspace e ferramentas.</div>
                </div>
              `
      : html`
              ${renderAgentHeader(
...
function renderAgentHeader(
  ...
  const subtitle = agent.identity?.theme?.trim() || "Workspace do agente e roteamento.";
...
function renderAgentOverview(params: {
  ...
  return html`
    <section class="card">
      ...
      <div class="card-sub">Caminhos do workspace e metadados de identidade.</div>
      <div class="agents-overview-grid" style="margin-top: 16px;">
        <div class="agent-kv">
          <div class="label">Espaço de Trabalho</div>
        ...
        <div class="agent-kv">
          <div class="label">Nome da Identidade</div>
        ...
        <div class="agent-kv">
          ...
          <div>${isDefault ? "sim" : "não"}</div>
        </div>
        <div class="agent-kv">
          <div class="label">Emoji da Identidade</div>
          <div>${identityEmoji}</div>
        </div>
        <div class="agent-kv">
          <div class="label">Filtro de Habilidades</div>
          <div>${skillFilter ? `${skillCount} selecionada(s)` : "todas as habilidades"}</div>
        </div>
      </div>

      <div class="agent-model-select" style="margin-top: 20px;">
        <div class="label">Seleção de Modelo</div>
        <div class="row" style="gap: 12px; flex-wrap: wrap;">
          <label class="field" style="min-width: 260px; flex: 1;">
            <span>Modelo primário${isDefault ? " (padrão)" : ""}</span>
            <select
      ...
      : html`
                      <option value="">
                        ${defaultPrimary ? `Herdar padrão (${defaultPrimary})` : "Herdar padrão"
        }
                      </option>
                    `
    }
          ...
          <label class="field" style="min-width: 260px; flex: 1;">
            <span>Fallbacks (separados por vírgula)</span>
            <input
              .value=${fallbackText}
              ?disabled=${!configForm || configLoading || configSaving}
...
function renderAgentContextCard(context: AgentContext, subtitle: string) {
  return html`
    <section class="card">
      <div class="card-title">Contexto do Agente</div>
      <div class="card-sub">${subtitle}</div>
      <div class="agents-overview-grid" style="margin-top: 16px;">
        <div class="agent-kv">
          <div class="label">Espaço de Trabalho</div>
        ...
        <div class="agent-kv">
          <div class="label">Padrão</div>
          <div>${context.isDefault ? "sim" : "não"}</div>
        </div>
      </div>
    </section>
  `;
...
function renderAgentChannels(params: {
  ...
  const lastSuccessLabel = params.lastSuccess ? formatAgo(params.lastSuccess) : "nunca";
  return html`
    <section class="grid grid-cols-2">
      ${renderAgentContextCard(context, "Configuração de workspace, identidade e modelo.")}
      <section class="card">
        <div class="row" style="justify-content: space-between;">
          <div>
            <div class="card-title">Canais</div>
            <div class="card-sub">Snapshot de status de canais do gateway.</div>
          </div>
          <button class="btn btn--sm" ?disabled=${params.loading} @click=${params.onRefresh}>
            ${params.loading ? "Atualizando…" : "Atualizar"}
          </button>
        </div>
        <div class="muted" style="margin-top: 8px;">
          Última atualização: ${lastSuccessLabel}
        </div>
        ${params.error
      ...
      : nothing
        ...
        ${!params.snapshot
      ? html`
                <div class="callout info" style="margin-top: 12px">Carregue canais para ver status ao vivo.</div>
              `
      : nothing
    }
        ${entries.length === 0
      ? html`
                <div class="muted" style="margin-top: 16px">Nenhum canal encontrado.</div>
              `
      : html`
        ...
        const status = summary.total
          ? `${summary.connected}/${summary.total} conectado(s)`
          : "sem contas";
        const config = summary.configured
          ? `${summary.configured} configurado(s)`
          : "não configurado";
        const enabled = summary.total ? `${summary.enabled} ativado(s)` : "desativado";
...
function renderAgentCron(params: {
  ...
  return html`
    <section class="grid grid-cols-2">
      ${renderAgentContextCard(context, "Workspace e alvos de agendamento.")}
      <section class="card">
        <div class="row" style="justify-content: space-between;">
          <div>
            <div class="card-title">Agendador</div>
            <div class="card-sub">Status cron do gateway.</div>
          </div>
          <button class="btn btn--sm" ?disabled=${params.loading} @click=${params.onRefresh}>
            ${params.loading ? "Atualizando…" : "Atualizar"}
          </button>
        ...
        <div class="stat-grid" style="margin-top: 16px;">
          <div class="stat">
            ...
            <div class="stat-value">
              ${params.status ? (params.status.enabled ? "Sim" : "Não") : "n/a"}
            </div>
          </div>
          <div class="stat">
    ...
    <section class="card">
      <div class="card-title">Tarefas Cron do Agente</div>
      <div class="card-sub">Tarefas agendadas para este agente.</div>
      ${jobs.length === 0
      ? html`
              <div class="muted" style="margin-top: 16px">Nenhuma tarefa atribuída.</div>
            `
      : html`
              <div class="list" style="margin-top: 16px;">
                ${jobs.map(
                    ...
                    <div class="list-main">
                      ...
                      <div class="chip-row" style="margin-top: 6px;">
                        ...
                        <span class="chip ${job.enabled ? "chip-ok" : "chip-warn"}">
                          ${job.enabled ? "ativado" : "desativado"}
                        </span>
...
function renderAgentFiles(params: {
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">Arquivos Principais</div>
          <div class="card-sub">Persona de inicialização, identidade e guia de ferramentas.</div>
        </div>
        <button
          class="btn btn--sm"
        ...
        >
          ${params.agentFilesLoading ? "Carregando…" : "Atualizar"}
        </button>
      </div>
      ${list ? html`<div class="muted mono" style="margin-top: 8px;">Workspace: ${list.workspace}</div>` : nothing}
      ${params.agentFilesError
      ? html`<div class="callout danger" style="margin-top: 12px;">${params.agentFilesError
        }</div>`
      : nothing
    }
      ${!list
      ...
      : html`
              <div class="agent-files-grid" style="margin-top: 16px;">
                <div class="agent-files-list">
                  ${files.length === 0
          ? html`
                          <div class="muted">Nenhum arquivo encontrado.</div>
                        `
          : files.map((file) =>
            renderAgentFileRow(file, active, () => params.onSelectFile(file.name)),
                ...
                <div class="agent-files-editor">
                  ${!activeEntry
          ? html`
                          <div class="muted">Selecione um arquivo para editar.</div>
                        `
          : html`
                          <div class="agent-file-header">
                            ...
                            <div class="agent-file-actions">
                              ...
                              >
                                ${params.agentFileSaving ? "Salvando…" : "Salvar"}
                              </button>
                            </div>
                          </div>
                          ${activeEntry.missing
              ? html`
                                  <div class="callout info" style="margin-top: 10px">
                                    Este arquivo está faltando. Salvar irá criá-lo no workspace do agente.
                                  </div>
                                `
              : nothing
            }
...
function renderAgentFileRow(file: AgentFileEntry, active: string | null, onSelect: () => void) {
  ...
  return html`
    ...
    >
      ...
      ${file.missing
      ? html`
              <span class="agent-pill warn">faltando</span>
            `
      : nothing
    }
    </button>
...
function renderAgentTools(params: {
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">Acesso a Ferramentas</div>
          <div class="card-sub">
            Perfil + substituições por ferramenta para este agente.
            <span class="mono">${enabledCount}/${toolIds.length}</span> ativado(s).
          </div>
        </div>
        <div class="row" style="gap: 8px;">
          ...
          >
            ${params.configSaving ? "Salvando…" : "Salvar"}
          </button>
        </div>
      </div>
      ...
      ? html`
              <div class="callout info" style="margin-top: 12px">
                Carregue a config do gateway para ajustar perfis de ferramentas.
              </div>
            `
      : nothing
    }
      ...
      ? html`
              <div class="callout info" style="margin-top: 12px">
                Este agente usa uma allowlist explícita na config. Substituições de ferramentas são gerenciadas na aba Config.
              </div>
            `
      : nothing
    }
      ...
      ? html`
              <div class="callout info" style="margin-top: 12px">
                tools.allow global definido. Substituições de agente não podem ativar ferramentas bloqueadas globalmente.
              </div>
            `
      : nothing
    }
      ...
      <div class="agent-tools-presets" style="margin-top: 16px;">
        <div class="label">Predefinições Rápidas</div>
...
const SKILL_SOURCE_GROUPS: Array<{ id: string; label: string; sources: string[] }> = [
  { id: "workspace", label: "Habilidades do Workspace", sources: ["openclaw-workspace"] },
  { id: "built-in", label: "Habilidades Nativas", sources: ["openclaw-bundled"] },
  { id: "installed", label: "Habilidades Instaladas", sources: ["openclaw-managed"] },
  { id: "extra", label: "Habilidades Extras", sources: ["openclaw-extra"] },
];
...
function groupSkills(skills: SkillStatusEntry[]): SkillGroup[] {
  ...
  const other: SkillGroup = { id: "other", label: "Outras Habilidades", skills: [] };
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          ...
          <div class="card-sub">
            Lista de permissão de habilidades por agente e habilidades do workspace.
        ...
        <div class="row" style="gap: 8px;">
          ...
          <button class="btn btn--sm" ?disabled=${params.loading} @click=${params.onRefresh}>
            ${params.loading ? "Carregando…" : "Atualizar"}
          </button>
          <button
            class="btn btn--sm primary"
            ?disabled=${params.configSaving || !params.configDirty}
      ...
      ${usingAllowlist
      ? html`
              <div class="callout info" style="margin-top: 12px">Este agente usa uma lista de permissão de habilidades personalizada.</div>
            `
      : html`
              <div class="callout info" style="margin-top: 12px">
                Todas as habilidades estão ativadas. Desativar qualquer habilidade criará uma lista de permissão por agente.
              </div>
            `
    }
      ...
      ? html`
              <div class="callout info" style="margin-top: 12px">
                Carregue as habilidades para este agente para ver entradas específicas do workspace.
              </div>
            `
      : nothing
    }
      ...
      <div class="filters" style="margin-top: 14px;">
        ...
        <div class="muted">${filtered.length} exibido(s)</div>
      </div>
...
function renderAgentSkillRow(
  ...
  if (skill.disabled) {
    reasons.push("desativado");
  }
  if (skill.blockedByAllowlist) {
    reasons.push("bloqueado pela lista de permissão");
  }
  return html`
      ...
      <div class="list-main">
        ...
        <div class="chip-row" style="margin-top: 6px;">
          ...
          <span class="chip ${skill.eligible ? "chip-ok" : "chip-warn"}">
            ${skill.eligible ? "elegível" : "bloqueado"}
          </span>
          ${skill.disabled
      ? html`
                  <span class="chip chip-warn">desativado</span>
                `
      : nothing
    }
        </div>
        ${missing.length > 0
      ? html`<div class="muted" style="margin-top: 6px;">Faltando: ${missing.join(", ")}</div>`
      : nothing
    }
        ${reasons.length > 0
      ? html`<div class="muted" style="margin-top: 6px;">Motivo: ${reasons.join(", ")}</div>`
      : nothing
    }
      </div>
      <div class="list-meta">
